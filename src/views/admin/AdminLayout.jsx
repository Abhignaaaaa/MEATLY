...
import { LayoutDashboard, Users, Store, Package, ShoppingBag, LogOut, Menu, X } from 'lucide-react';
...
export default function AdminLayout({ currentView, onNavigate, children }) {
  ...
  const navItems = [
    ...
    { label: 'Shops', icon: Store, view: 'admin-shops' },
    { label: 'Products', icon: Package, view: 'admin-products' },
    { label: 'Orders', icon: ShoppingBag, view: 'admin-orders' }
  ];

  const SidebarContent = () => (
    <>
      <div className="p-6">
        <h1 className="text-2xl font-black text-indigo-700 tracking-tighter">MEATLY <span className="text-[#20231B]">ADMIN</span></h1>
        <p className="text-xs text-[#6F7268] mt-1">Logged in as {user?.fullName}</p>
      </div>
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-[12px] text-sm font-semibold transition-colors ${
              currentView.startsWith(item.view) 
                ...
                : 'text-[#6F7268] hover:bg-indigo-50 hover:text-indigo-900'
            }`}
          >
  ...
  return (
    <div className="min-h-screen bg-[#F7F8EF] flex">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white border-b border-[#E4E4DA] z-30 flex items-center justify-between px-4">
        <h1 className="text-xl font-black text-indigo-700 tracking-tighter">MEATLY <span className="text-[#20231B]">ADMIN</span></h1>
